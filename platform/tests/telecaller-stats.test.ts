...
/**
 * The productivity rollup's SQL (migration 0090), against a real Postgres.
 *
...
describe("telecaller productivity rollup (0090)", () => {
  beforeAll(async () => {
  ...
  it("reports the active span from the first call to the last", async () => {
    const row = await statsFor(TENANT_A.orgId, TELECALLER_A);
    // 09:00:00 to 10:05:10 = 3910s. A span, not time worked - see 0090.
