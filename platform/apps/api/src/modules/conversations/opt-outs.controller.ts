      ...
      const { rows } = await client.query(
        `SELECT o.id, o.channel, o.peer_address, o.created_at,
           ...
           LEFT JOIN conversations c
                  ON c.org_id = o.org_id AND c.channel = o.channel
                 AND c.peer_address = o.peer_address
           LEFT JOIN conversation_messages m ON m.id = o.source_message_id
          WHERE o.org_id = $1 AND o.level = 'probable'
            AND o.released_at IS NULL AND o.reviewed_at IS NULL
          ORDER BY o.created_at DESC
          LIMIT $2`,
        [orgId, parsed.data.limit],
      );
      const {
        rows: [count],
