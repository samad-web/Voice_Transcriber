...
function fakeDb(
  granted: boolean,
  scope: "all" | "owned" = "all",
  // The persona the membership row carries (migration 0079). Defaults to
  // `null`, which is exactly what every row looked like before 0018 backfilled
  // them - `resolveOwnerRole(null)` is `owner`, so the default case is the
  // pre-persona behaviour, unchanged.
  ownerRole: string | null = null,
): { db: DbService; calls: Recorded[] } {
    ...
    withOrg: async (orgId: string, fn: (client: unknown) => Promise<unknown>) => {
      const client = {
        query: async (_sql: string, params: unknown[]) => {
          ...
          return { rows: granted ? [{ scope, owner_role: ownerRole }] : [] };
        },
      };
      return fn(client);
    },
