...
interface EnrichInputs {
  ...
  /** Copied onto the SOP result for the same reason - see 0089's call_started_at. */
  startedAt: Date | null;
}
...
export async function enrichCall({ callId, orgId }: PipelineMessage): Promise<void> {
  ...
  try {
    loaded = await withOrgContext(orgId, async (client) => {
      const {
        ...
        `SELECT t.text, t.segments, t.diarized, c.direction, c.telecaller_id AS "telecallerId",
       ...
       *
       * Gated for exactly the reason the talk metrics below are (0083, and
       ...
       * commercial weight. A false pass there is far worse than no score.
       */
  ...
  try {
    await withOrgContext(orgId, async (client) => {
      ...
      /*
       * SOP adherence (0089). Non-blocking, like the analytics above: a lead is
