  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border bg-surface px-4 py-3 md:hidden">
        <div className="flex h-9 w-9 shrink-0 select-none items-center justify-center rounded-md bg-text text-base font-semibold text-bg">
          A
        </div>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex w-full items-center gap-3 rounded-md px-3 py-3 text-sm font-medium transition-colors duration-150 ease-out ${
                        // Same sanctioned accent use as <Sidebar>; the two rails
                        // must agree on what "you are here" looks like.
                        isActive
