import { featureEnabled, type OrgFeature, type OwnerRole } from "@aura/shared";
...
export interface ReviewSourceSpec {
  ...
  blurb: string;
  /** The org feature (0093) the source's own page is gated on, if any. */
  feature: OrgFeature | null;
  /**
...
export function reviewSourcesFor(
  ...
  return REVIEW_SOURCES.filter(
    (s) =>
      ...
      (s.feature === null || featureEnabled(s.feature, [...modules], [...features])),
  ).map((s) => s.key);
}
