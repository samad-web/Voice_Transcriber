...
import { featureForHref, featureEnabled, type OwnerRole } from "@aura/shared";
...
function itemAllowedByFeatures(href: string, entitlement?: Entitlement): boolean {
  ...
  const spec = featureForHref(href);
  if (!spec) return true;
  return featureEnabled(spec.id, entitlement.modules, entitlement.features);
}
