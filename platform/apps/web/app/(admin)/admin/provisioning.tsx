  ...
  return (
    <Card elevated className="overflow-hidden p-0">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-bg-subtle px-5 py-3.5">
        ...
        <span className="text-xs text-text-muted">modules, features and WhatsApp per client</span>
        <Button
          type="button"
          size="sm"
          className="ml-auto"
      ...
      {tenants.length === 0 ? (
        ...
        <div tabIndex={0} role="region" aria-label="Provisioning" className="overflow-x-auto">
          <table className="w-full min-w-[760px] border-collapse text-left text-sm">
            <caption className="sr-only">Per-tenant modules, features and WhatsApp provider</caption>
            <thead className="bg-bg-subtle">
              <tr>
                {["Tenant", "Modules", "Features", "WhatsApp", ""].map((h) => (
                  <th
                    key={h}
                    scope="col"
            ...
            <tbody className="divide-y divide-border">
              {tenants.map((t) => {
                const live = ORG_FEATURES.filter(
                  (f) => t.enabled_modules.includes(f.module) && t.enabled_features.includes(f.id),
                ).length;
                const possible = ORG_FEATURES.filter((f) =>
                  t.enabled_modules.includes(f.module),
                ).length;
                ...
                return (
                  <tr key={t.id} className="transition-colors duration-150 hover:bg-surface-hover">
                    ...
                    <td className="px-4 py-3 align-middle text-text tabular-nums">
                      {live}/{possible}
                    </td>
                    <td className="px-4 py-3 align-middle">
...
function ModuleFeatureGrid({
  ...
  onFeatures: (next: OrgFeature[]) => void;
}) {
  const byModule = useMemo(
    () => ORG_MODULES.map((m) => ({ module: m, items: ORG_FEATURES.filter((f) => f.module === m.id) })),
    [],
  );
  ...
  const toggleModule = (id: OrgModule, on: boolean) => {
    ...
    if (id === "aura") return;
    const next = on ? [...modules, id] : modules.filter((m) => m !== id);
    onModules(next);
    // Turning a module ON grants its default features rather than leaving the
    // client an empty section they are entitled to; turning it OFF drops them,
    // which is what the API will store anyway.
    onFeatures(
      on
        ? Array.from(new Set([...features, ...defaultFeaturesFor([id])]))
        : reconcileFeatures(next, features),
    );
  };
  ...
  return (
    <div className="space-y-5">
            ...
            {items.length > 0 ? (
              <div className="mt-2 ml-6 grid gap-1.5 sm:grid-cols-2">
                {items.map((f) => (
                  <Checkbox
                    key={f.id}
                    checked={on && features.includes(f.id)}
                    disabled={!on}
                    onChange={(e) =>
                      onFeatures(
                        e.target.checked
      ...
      <RowHint kind="blocked">
        Feature toggles control what a client SEES - the page leaves their sidebar and stops
        rendering. They are not a security boundary: the module entitlement and the role
        permission grid are what the API enforces on every request, and neither is changed by
        hiding a page. Nothing stored is deleted either - switch Invoices off and the invoices
...
function CreateEnvironment({ onClose }: { onClose: () => void }) {
  ...
  const [features, setFeatures] = useState<OrgFeature[]>(defaultFeaturesFor(["aura", "crm"]));
    ...
    >
      {result ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            <MonoLabel>Modules and features</MonoLabel>
            <div className="mt-2">
              <ModuleFeatureGrid
                modules={modules}
                features={features}
...
function ConfigureTenant({
  ...
  const [features, setFeatures] = useState<OrgFeature[]>(tenant.enabled_features as OrgFeature[]);
