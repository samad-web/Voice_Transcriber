...
export default async function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  // The finer axis (migration 0093): which FEATURES inside those modules an
  // operator has provisioned for this client. Passed to both rails so a
  // switched-off page leaves the navigation entirely rather than 404ing when
  // somebody clicks it - the page-level `requireOwnerFeature` is the backstop
  // for a bookmark, not the primary experience.
  const entitlement = {
  ...
  // render. Every existing org was backfilled as complete by 0095, so this is
