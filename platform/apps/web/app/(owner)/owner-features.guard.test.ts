...
describe("feature gating", () => {
  it("has a page file for every route the catalogue governs", () => {
    ...
    const missing = FEATURES.flatMap((f) =>
      f.hrefs.filter((href) => !existsSync(pageFor(href))).map((href) => `${f.key}: ${href}`),
    );
  ...
  it("calls requireOwnerFeature with the right id on every governed page", () => {
    ...
    // `locked` features cannot be switched off (leads, staff), so there is no
    // "off" for a page guard to enforce - the same exemption
    // lib/feature-gating.test.ts makes.
    for (const feature of FEATURES.filter((f) => !f.locked)) {
      for (const href of feature.hrefs) {
        ...
        //
        // Two gate forms are legal and mean the same thing: `requireFeature`
        // keyed by PATH (owner-context.ts) and `requireOwnerFeature` keyed by
        // FEATURE (owner-features.ts). Both resolve through the one
        // `enabledFeatures` in @aura/shared, so a page may use whichever reads
        // better at its call site - but it must use one of them.
        const code = blankNonCode(raw);
        const byFeature =
          code.includes("requireOwnerFeature(") &&
          raw.includes(`requireOwnerFeature("${feature.key}")`);
        const byPath = code.includes("requireFeature(") && raw.includes(`requireFeature("${href}")`);
        if (!byFeature && !byPath) {
          offenders.push(
            `${href} should call requireFeature("${href}") or requireOwnerFeature("${feature.key}")`,
          );
        }
      }
    }
  ...
  it("gates BEFORE the page fetches anything", () => {
    ...
    for (const feature of FEATURES) {
      for (const href of feature.hrefs) {
        ...
        const byFeatureAt = body.indexOf("requireOwnerFeature");
        const byPathAt = body.indexOf("requireFeature");
        const gate = byFeatureAt >= 0 ? byFeatureAt : byPathAt;
  ...
  it("governs a route the owner navigation actually has", () => {
    ...
    // A feature with NO hrefs is a panel on somebody else's page rather than a
    // destination of its own (sheets_sync). It cannot orphan a route it never
    // claimed, so it is not a candidate here.
    const orphans = FEATURES.filter(
      (f) => f.hrefs.length > 0 && !f.hrefs.some((h) => navHrefs.has(h)),
    ).map((f) => f.key);
  ...
  it("hides a page from the rail when its feature is off", () => {
    ...
    // Everything on, except optionally one - the sparse-override shape the
    // client's own switchboard writes.
    const on = (except?: FeatureKey): FeatureOverrides =>
      Object.fromEntries(FEATURES.map((f) => [f.key, f.key !== except]));
  ...
  it("leaves the rail untouched when no entitlement is passed", () => {
    ...
    // existed before migration 0101 keeps the module-only answer, so adding
