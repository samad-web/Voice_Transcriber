...
describe("what the client was sold", () => {
  it("lists only the features their entitlement actually includes", () => {
    const line = text({ ...RUNNING, modules: ["aura", "crm"], features: only("inbox") }, "included");
  ...
  it("drops the line entirely for a tenant on core Aura alone", () => {
    ...
    expect(ids({ ...RUNNING, modules: ["aura"], features: only() })).not.toContain("included");
  });

  it("reads as a sentence for one, two and several", () => {
    const one = text({ ...RUNNING, modules: ["aura", "crm"], features: only("inbox") }, "included");
    expect(one).toBe("Included on your plan: the shared inbox");

    const two = text(
      { ...RUNNING, modules: ["aura", "crm", "wasi"], features: only("inbox", "messaging_setup") },
      "included",
    );
