 ...
 * provisioned it (migration 0101). Telling somebody to connect Meta Lead Ads
...
export interface SetupStepSpec {
  ...
  required: boolean;
  /**
   * Hidden entirely when the org lacks this feature (migration 0101). Absent
...
export function setupStepsFor(entitlement: SetupEntitlement): SetupStepSpec[] {
  ...
  return SETUP_STEPS.filter((step) => !step.feature || on.has(step.feature));
}
