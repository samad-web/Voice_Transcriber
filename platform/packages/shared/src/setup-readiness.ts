 ...
 *
 * The setup modal (migration 0106) opens with a list of things that are NOT
...
export function readinessLines(facts: ReadinessFacts): ReadinessLine[] {
  ...
  const on = enabledFeatures(facts.modules, facts.features);
  const extras = EXTRAS.filter((e) => on.has(e.feature));
  if (extras.length > 0) {
    lines.push({
      id: "included",
...
const EXTRAS: Array<{ feature: FeatureKey; label: string }> = [
