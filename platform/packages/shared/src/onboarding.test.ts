...
const BARE: SetupEntitlement = { modules: ["aura"], features: [] };
...
describe("the catalogue", () => {
  ...
  it("names a real feature wherever it gates one", () => {
    const known = new Set(ORG_FEATURES.map((f) => f.id));
    for (const step of SETUP_STEPS) {
  ...
  it("marks the handset step required, and points it at a page that can finish it", () => {
    // It shipped guided-only in 0095 because pairing needed the operator.
    // Migration 0096 gave the client /owner/devices, which is what made it
