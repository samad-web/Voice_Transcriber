...
/**
 * Automated lead distribution (migration 0094) - the half that touches state.
 *
...
async function route(
  ...
  const { rows: ruleRows } = await client.query<RuleRow>(
    ...
    // `deleted_at IS NULL` (0097) is load-bearing, not hygiene: a rule the
  ...
  const {
    ...
    // The `deleted_at` guard is what keeps this check meaningful after 0097.
  ...
  const { rows: targetRows } = await client.query<TargetRow>(
    ...
    // timezone (0088), not the server's. A daily cap that rolls over at
